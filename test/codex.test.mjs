...
test('Slack prompts: guide someone to link their own tool when they have not, and use only their access', () => {
  ...
  assert.match(p, /hasn't linked their Linear and Notion yet[\s\S]*\/link linear[\s\S]*node deploy\/ctl\.mjs link <notion> <their Slack id>[\s\S]*use their own access, never anyone else's/)
