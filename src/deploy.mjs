...
const SENSITIVE = [/^src\/(access|publish|gitpush|prgrant|githubapp|proxy|chatgpt|main|controller|deploy|mentions|webhook|session|state|github|reply|policy|tools|oauth|userlogins|slack|slack-channel|slack-events|slack-socket|slack-reply)\.mjs$/, /^box\//, /^deploy\//]
