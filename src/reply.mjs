 ...
 * something went wrong), and a dropped connection leaves it unknown: then it looks for this very
 * reply — by an invisible marker, since two replies can read the same — before posting it again.
 */
export async function reply(gh, req, text, { footer = true, retryDelayMs = 2000 } = {}) {
  const t = String(text || '').trim() || '(no answer)'
  const mark = `<!-- botlite:${randomUUID()} -->`
  ...
  try {
    ...
    if (e.status < 500) throw e // GitHub said no: nothing was posted
    await new Promise((r) => setTimeout(r, retryDelayMs))
    const posted = (await gh.json('GET', `${list}?since=${since}&per_page=100`)).find((c) => c.body?.includes(mark))
